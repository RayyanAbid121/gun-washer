...
export const Homepage = () => {
  ...
  return (
    <div className="relative h-screen w-full overflow-hidden bg-white font-roboto">
      {/* Background line — matches Figma Vector 1: 1446x1264 @ top:-96 left:-9
          on a 1440x1024 frame, pure #FF7A00 at 30% opacity, no darkening filter */}
      <img
        src={back1Image}
        alt=""
        aria-hidden="true"
        className="pointer-events-none absolute -left-[0.6%] -top-[9.4%] h-[123.4%] w-[100.4%] max-w-none object-cover opacity-30"
      />
      ...
      <div className="relative z-10 mx-auto flex h-full w-full max-w-[1440px] flex-col items-center px-6 py-5 lg:px-16">
        ...
        <motion.h1
          ...
          className="mt-3 max-w-5xl shrink-0 text-center text-3xl font-bold leading-tight text-balance text-gray-900 lg:text-5xl"
        >
        ...
        {/* Description + rating — pinned bottom-left */}
        <div className="mt-auto w-full max-w-xs shrink-0 self-start pb-4">
          ...
          >
            {[...Array(5)].map((_, i) => (
              <span key={i} className="text-lg text-[#FF7A00]">
                ★
              </span>
            ))}
          </motion.div>
      ...
      {/* Curtain intro — two orange panels covering everything, splitting
          apart from center to reveal the page underneath, plays once */}
      {!introDone && (
        <>
          <motion.div
