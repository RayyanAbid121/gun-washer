...
function App() {
  ...
  // HomeDetail(stage 3). Only steps that change *page* (index) trigger the
  // full slide transition; a stage flip inside HomeDetail is handled
  // internally by that component and doesn't remount/slide the page.
  const goDirection = useCallback(
    (dir: 1 | -1) => {
      ...
      if (dir === 1) {
        ...
        // index===1 && detailStage===3: already at the end, no-op
      } else {
        if (index === 1 && detailStage === 3) {
